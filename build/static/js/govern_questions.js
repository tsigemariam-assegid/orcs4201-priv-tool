// static/js/govern_questions.js
$(document).ready(() => {
  const $container   = $('#questions-container');
  const category     = $container.data('category');      // "Govern"
  const introUrl     = $container.data('intro-url');     // "/govern"
  const completeUrl  = $container.data('complete-url');  // "/control"
  ...
  fetch('/static/nist1.json')
    ...
    .then(data => {
          ...
          <div class="question-box">
            <div class="question-content">
              ...
              <button class="next-btn">
                ${qIdx === total - 1 ? 'Finish Govern' : 'Next'}
              </button>
            </div>
      ...
      // restore previous selections & scores
