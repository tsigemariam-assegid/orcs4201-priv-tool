...
$(document).ready(() => {
    fetch('/static/nist.json')
      .then(res => res.json())
      .then(data => {
        const info = data.find(item => item.category === 'Identify');
        if (!info) return console.error('No "Identify" entry found');
        ...
        const $credit = $(credit).hide();          // create & hide credit
        ...
        $('#insights-toggle').on('click', function() {
          ...
          // if closing, hide credit immediately
          if (isOpen) {
            $credit.hide();
          }
          ...
          $panel.slideToggle(600, 'swing', function() {
            ...
            // if opening, fadeIn credit after slide
            if (nowOpen) {
              $credit.fadeIn(400);
            }
          });
